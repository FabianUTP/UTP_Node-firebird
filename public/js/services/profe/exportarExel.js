function descargarTablaExcel() {
    try {
      ...
      const nombreProfesorElement = document.querySelector("h4:nth-of-type(1)"); // Primer h4 es el profesor
      ...
      const data = rows.map(row => {
        ...
        // Asegurarse de que hay 4 celdas (incluyendo calificación) en cada fila
        if (cells.length === 4) {
          return cells.map(cell => cell.textContent.trim());
        }
        return null; // Si la fila no tiene 4 celdas, la descartamos
      }).filter(row => row !== null); // Filtrar las filas vacías o inválidas
  
      // Verificar si los datos se han extraído correctamente
      if (data.length === 0) {
